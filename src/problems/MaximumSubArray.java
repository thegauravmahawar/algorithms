...
public class MaximumSubArray {
     ...
     * @return int
     * <p>
     * The idea that we have used here is that, if we look at [-2, 1] - the problem says maximum sub array can contain only
     * one element also. So, for the above mentioned array, there are 3 possibilities - -2, -2 + 1, 1, clearly 1 is the largest.
     * <p>
     * The above idea is especially important when the following values are smaller, for example, [4, -1, 2, 1]. Initially the subArraySum
     * will be 4, but when the index reaches -1, the max value is taken from the comparison of -1 and 4 + (-1), which is 3, so subArraySum
     ...
     * <p>
     * So, we will use a variable (subArraySum) to keep track of the sum of all the previous elements before the current element, however,
     * when the current element is greater than the combined sum of all previous elements, we will use this current element as
     * the max sum found till now and assign it to subArraySum. We copy this value over to our final sum, which we want to
     * return if the subArraySum is greater than the final sum.
     */
